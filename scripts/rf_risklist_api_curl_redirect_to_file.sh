...
# OR 
# CURLARGS="-H "X-RFToken: $RFAPITOKEN" -f -o iprisklist_$(date +"%Y_%m_%d_%H_%M_%S_%s").csv""
# Enable the below statement by removing the comment and sniff the traffic for troubleshooting:
# CURLARGS="-H "X-RFToken: $RFAPITOKEN" -f -p 8080""
...
#$CURL $CURLARGS $RFIPRISKLIST >> ./unparsed_iprisklist_$(date +"%Y_%m_%d_%H_%M_%S_%s").csv
